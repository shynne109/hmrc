 ...
 */
class P46CarTest extends TestCase
{
   ...
   private function buildEmployer(): ReportingCompany
   {
    ...
    /**
     * Test: P46 Car with HMRC Official Test Scenario Data
     * 
     * This test uses the official HMRC test scenario from their documentation:
     * - Employee: George Edgar Turner (NINO: RN000012)
     * - Car: Citroen C4 LX
     * - CO2: 47 g/km, Petrol fuel
     * - List Price: £7999
     * 
     * @see HMRC Business Rules for P46 Car (Expenses and Benefits section)
     */
    public function testP46CarWithHmrcScenarioData(): void
    {
        // Use tax year 25-26 as per current HMRC test environment requirements
        ...
        $p11d = new P11D('ISV635', 'fGuR34fAOEJf', $employer, '2026-04-05', true);
        $p11d->setLogger(new \Psr\Log\NullLogger());
        $p11d->setSoftwareMeta('9256', 'Abbpay Solutions', '1.0.0');
        $p11d->setRelatedTaxYear('25-26');

        // HMRC Test Scenario Employee Data
        $car = new P46Car([
            'forename' => 'George',
            'secondForename' => 'Edgar',
            'surname' => 'Turner',
            'submissionReason' => 'New',
            'nino' => 'RN000012'
        ]);

        // HMRC Test Scenario Vehicle Data
        $car->setNino('RN000012');
        $car->setMakeAndModel('Citroen C4 LX');
        $car->setCo2Emissions(47);
        $car->setFuelType('A');  // All other (petrol)
        
        // For MonetaryDetails to appear, we need CarPrice, DateFirstAvailable, and CapitalContributions
        $car->setCarPrice(7999);
        $car->setDateFirstAvailable('2025-06-01');
        $car->setCapitalContributions(0);

        $p11d->addP46Car($car);
        $xml = $p11d->buildXML();

        // Validate the XML contains expected data
        $this->assertStringContainsString('George', $xml);
        $this->assertStringContainsString('Turner', $xml);
        $this->assertStringContainsString('Citroen C4 LX', $xml);
        $this->assertStringContainsString('<Emissions>47</Emissions>', $xml);
        $this->assertStringContainsString('<CarPrice>7999.00</CarPrice>', $xml);
        $this->assertStringContainsString('<FuelType>A</FuelType>', $xml);
    }

    /**
     * Test: P46 Car XML validates against schema namespace
     */
    public function testP46CarXmlHasCorrectNamespace(): void
    {
        $p11d = $this->buildP11D();
        $car = new P46Car([
            'forename' => 'Test',
        ...
        // Check for EXB namespace
        $this->assertStringContainsString('http://www.govtalk.gov.uk/taxation/EXB/', $xml);
    }

    /**
     * Test: P46 Car with zero-emission vehicle (electric)
     */
    public function testP46CarWithZeroEmissionVehicle(): void
    {
        $p11d = $this->buildP11D();
        $car = new P46Car([
            'forename' => 'Electric',
        ...
        $car->setCo2Emissions(0);
        $car->setFuelType('A');  // FuelType per XSD
        ...
        $this->assertStringContainsString('<FuelType>A</FuelType>', $xml);
        $this->assertStringContainsString('<Emissions>0</Emissions>', $xml);
    }

    /**
     * Test: P46 Car with all optional fields
     */
    public function testP46CarWithAllOptionalFields(): void
    {
        $p11d = $this->buildP11D();
        $car = new P46Car([
            'forename' => 'Complete',
        ...
        $car->setMakeAndModel('BMW 3 Series 320i');
        $car->setCo2Emissions(135);
        $car->setFuelType('A');
        
        // MonetaryDetails requires all three of these
        $car->setCarPrice(45000);
        $car->setDateFirstAvailable('2025-06-01');
        $car->setCapitalContributions(3000);
        $car->setPrivateUsePayment(200);

        $p11d->addP46Car($car);
        $xml = $p11d->buildXML();

        $this->assertStringContainsString('Complete', $xml);
        $this->assertStringContainsString('Record', $xml);
        $this->assertStringContainsString('BMW 3 Series 320i', $xml);
        $this->assertStringContainsString('<Emissions>135</Emissions>', $xml);
        $this->assertStringContainsString('<CarPrice>45000.00</CarPrice>', $xml);
        $this->assertStringContainsString('<CapitalContributions>3000.00</CapitalContributions>', $xml);
        $this->assertStringContainsString('<PrivateUsePayment', $xml);
    }
}
