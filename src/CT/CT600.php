...
{
    private string $utr;
    private string $periodEnd; // YYYY-MM-DD (IRheader PeriodEnd)
    private string $periodFrom; // CompanyTaxReturn PeriodCovered From
    private string $periodTo; // CompanyTaxReturn PeriodCovered To
    private string $companyName;
    private string $companyRegNo; // Companies House number
    private string $companyType = '6'; // numeric code per schema enumeration
    private string $returnType = 'new';
    private ?string $accountsReason = null; // if null we emit ThisPeriodAccounts yes
    private ?string $computationsReason = null; // if null we emit ThisPeriodComputations yes
    ...
    private float $corporationTaxRate = 19.0; // legacy single-rate convenience
    private array $financialYearRates = []; // [year => rate]
    private ?int $associatedCompanies = null; // total associated companies for marginal relief purposes
    private bool $startingOrSmallCompaniesRate = false;
    private ?array $associatedCompaniesFinancialYears = null; // ['firstYear'=>int,'secondYear'=>int]
    // Marginal relief parameters (defaults for FY 2023 onwards) adjustable
    private ?float $mrLowerLimit = 50000.0; // before associated companies & period length adjustment
    private ?float $mrUpperLimit = 250000.0;
    private float $mrFractionNumerator = 3.0; // standard fraction 3/200
    private float $mrFractionDenominator = 200.0;
    private bool $enableSchemaValidation = false;
    private ?string $localSchemaPath = null;
    private array $accountsAttachments = []; // each: ['mode'=>'inline'|'encoded'|'raw','content'=>string,'filename'=>?string,'entryPoint'=>bool]
    private array $computationsAttachments = [];
    private array $schedules = []; // supplementary pages raw XML blocks keyed by code (A..P)
    /**
    ...
    private function buildBody(): string
    {
        ...
        $xw->writeElement('ProfitsBeforeOtherDeductions', $this->wholeMoney($this->tradingProfits));
        $xw->startElement('ChargesAndReliefs');
        $xw->writeElement('ProfitsBeforeDonationsAndGroupRelief', $this->wholeMoney($this->tradingProfits));
        $xw->endElement();
        $xw->writeElement('ChargeableProfits', $this->wholeMoney($this->tradingProfits));
        ...
        $xw->writeElement('NetCorporationTaxLiability', $this->money($tax));
        $xw->writeElement('TaxChargeable', $this->money($tax));
        $xw->writeElement('TaxPayable', $this->money($tax));
        $xw->endElement();
